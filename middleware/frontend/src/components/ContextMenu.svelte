...
{#if $contextMenuState.visible}
  <div
    class="fixed z-[200] bg-kobser-surface border border-white/10 rounded-xl shadow-2xl py-1 min-w-[196px]"
  ...
  >
    {#each $contextMenuState.items as item}
      ...
      {:else}
        <button
          class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors {item.danger ? 'text-red-400 hover:bg-red-500/10' : 'text-kobser-text hover:bg-white/5'}"
