...
<!-- Panel -->
<div
  class="fixed inset-y-0 right-0 w-full sm:w-96 z-[160] flex flex-col bg-peel-surface border-l border-white/10 shadow-2xl transition-transform duration-300 ease-in-out"
...
>
  <div class="flex items-center justify-between px-5 py-4 border-b border-white/10 flex-shrink-0">
    ...
    <button
      ...
      class="w-9 h-9 flex items-center justify-center text-peel-muted hover:text-white transition-colors rounded-full hover:bg-white/10"
    >
    ...
    {#if current}
      <div class="px-4 pt-5 pb-3">
        ...
        <div class="flex items-center gap-3 p-2 rounded-xl bg-white/5">
          <img src={trackArtUrl(current, 64)} alt="" class="w-10 h-10 rounded-md object-cover flex-shrink-0 bg-peel-bg">
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium truncate text-peel-accent">{current.title ?? '—'}</p>
            <p class="text-xs text-peel-muted truncate">{current.artist ?? '—'}</p>
          </div>
        </div>
      </div>
    {/if}
    ...
    {#if upcoming.length}
      <div class="px-4 pb-4">
        <div class="flex items-center justify-between mb-3">
          ...
          <button on:click={clearUpcoming} class="text-xs text-peel-muted hover:text-red-400 transition-colors">Clear</button>
        </div>
        <div class="flex flex-col gap-0.5">
          {#each upcoming as track, i}
            ...
            <div
              class="flex items-center gap-2 p-2 rounded-xl hover:bg-white/5 group cursor-pointer transition-colors {dragOverIdx === qIdx && dragSrcIdx !== qIdx ? 'border-t-2 border-peel-accent' : ''}"
              draggable="true"
            ...
            >
              ...
              <img src={trackArtUrl(track, 64)} alt="" class="w-10 h-10 rounded-md object-cover flex-shrink-0 bg-peel-bg pointer-events-none">
              <div class="flex-1 min-w-0 pointer-events-none">
                <p class="text-sm font-medium truncate">{track.title ?? '—'}</p>
                <p class="text-xs text-peel-muted truncate">{track.artist ?? '—'}</p>
              </div>
              <button
                class="w-8 h-8 flex items-center justify-center text-peel-muted hover:text-white rounded-full hover:bg-white/10 transition-all opacity-0 group-hover:opacity-100 flex-shrink-0"
